...
/**
 * WP-13: web-layer test for {@link RecurringOrderController}.
 *
 * <p>Uses {@code standaloneSetup} (matching {@code OrderControllerWebMvcTest}) so the
 * controller is exercised with the real {@code OrderServiceExceptionHandler}, bean
 * validation and the default Jackson JSON binding (jsr310 module is on the web
 * starter's classpath, so {@code LocalDateTime} round-trips), but without the Spring
 * Security filter chain. A JWT is injected as a request attribute and resolved into
 * the {@code @AuthenticationPrincipal}.
 */
